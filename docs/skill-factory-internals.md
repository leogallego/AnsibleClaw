...
The metadata is transformed into a template context -- the 6 variables that all Jinja2 templates receive:
...
All 5 templates receive the same 6 variables. Each picks the ones it needs:

#### skill_template.md → SKILL.md

| Variable | Where it appears |
|----------|-----------------|
| `module_name` | Title, CLI examples, AAP curl body (`"module_name": "ansible.builtin.package"`), inventory section |
| `skill_name` | YAML frontmatter (`name: ansible-package`), "When to Use" prose, AAP job template name suggestion (`package-deploy`) |
...
| `example_args` | CLI quick examples, AAP ad-hoc examples, AAP curl body |
...
The AAP section checks `AAP_CONTROLLER_URL` env var, token, controller reachability via `/api/v2/ping/`, and that `aap_run.py` exists.
